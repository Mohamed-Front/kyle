...
<template>
  <div v-can="'list permissions'" class="permissions-container">
    <div class="card">
      <div class="header">
        <h1>{{ $t('permission.permissions_Management') }}</h1>
        <p class="subtitle">{{ $t('permission.view_and_manage_all_system_permissions_grouped_by_modules') }}</p>
      </div>

      <div class="toolbar">
        <Button icon="pi pi-refresh" :label="$t('permission.refresh')" @click="fetchData" :loading="loading" />
      </div>
      ...
      <TabView v-model:activeIndex="activeTab" class="grouped-tabs">
        <TabPanel v-for="group in groupedPermissions" :key="group.value" :header="group.name">
          <DataTable :value="group.permissions" stripedRows class="p-datatable-sm"
            ...
            <Column field="id" :header="$t('permission.id')" sortable style="width: 80px"></Column>
            <Column field="name" :header="$t('permission.name')" sortable>
              <template #body="{data}">
                <Tag :value="data.name"
            ...
            <Column :header="$t('permission.actions')" style="width: 100px">
              <template #body="{data}">
            ...
            <template #empty>
              <div class="empty-message">
                ...
                <p>{{ $t('permission.no_permissions_found_in_this_group') }}</p>
              </div>
            </template>
          </DataTable>
        </TabPanel>
      ...
      <Dialog v-model:visible="visibleDialog" modal :header="$t('permission.update_Permission_Description')"
              :style="{ width: '50vw' }" :breakpoints="{ '960px': '75vw', '640px': '90vw' }">
        <div class="p-fluid">
          <div class="p-field">
            <label for="permissionName">{{ $t('permission.permission_Name') }}</label>
            <InputText id="permissionName" v-model="currentPermission.name" disabled />
          </div>
          <div class="p-field">
            <label for="description">{{ $t('permission.description') }}</label>
            <Textarea id="description" v-model="currentPermission.description"
                     rows="5" autoResize :placeholder="$t('permission.enter_permission_description')" />
          </div>
        </div>
        <template #footer>
          <Button :label="$t('permission.cancel')" icon="pi pi-times" @click="visibleDialog = false"
                  class="p-button-text" />
          <Button :label="$t('permission.save')" icon="pi pi-check" @click="updateDescription"
