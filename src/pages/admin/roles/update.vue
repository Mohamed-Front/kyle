...
// Group permissions by their module (extracted from name prefix)
const groupPermissions = (permissions) => {
  const groups = {};

  permissions.forEach(permission => {
    // Extract module name from permission name (e.g., "list users" -> "users")
    const moduleMatch = permission.name.match(/^(list|show|create|edit|delete)\s+(.+)/);
    const moduleName = moduleMatch ? moduleMatch[2] : 'other';
    ...
    groups[moduleName].permissions.push(permission);
  });
...
const groupedPermissions = computed(() => {
  return groupPermissions(permissionsData.value);
});

const toggleGroup = (group) => {
  const groupIds = group.permissions.map(p => p.id);

  if (isGroupSelected(group.value)) {
...
const isGroupSelected = (groupName) => {
  const group = groupedPermissions.value.find(g => g.value === groupName);
  if (!group || group.permissions.length === 0) return false;
  return group.permissions.every(item => selectedPermissions.value.includes(item.id));
};
...
const isGroupIndeterminate = (groupName) => {
  ...
  const hasSome = group.permissions.some(item => selectedPermissions.value.includes(item.id));
  const hasAll = group.permissions.every(item => selectedPermissions.value.includes(item.id));
...
<template>
  <div v-can="'edit roles'" class="permission-manager">
    <h1>{{ isEditing ? $t('role.edit') : $t('role.create') }}</h1>
    ...
    <div v-if="loading" class="loading">
      {{ isEditing ? $t('role.loading.roleData') : $t('role.loading.permissions') }}
    </div>
    ...
    <template v-else>
      ...
      <!-- Table View -->
      <div v-else-if="tableView" class="table-view">
        <table>
          <thead>
            <tr>
              <th>{{ $t('role.group') }}</th>
          ...
          <tbody>
            <template v-for="group in groupedPermissions" :key="group.value">
              <tr v-for="permission in group.permissions" :key="permission.id">
                <td v-if="group.permissions.indexOf(permission) === 0" :rowspan="group.permissions.length">
                  <input
                    type="checkbox"
                    :id="`group-${group.value}-table`"
                    :checked="isGroupSelected(group.value)"
                    :indeterminate.prop="isGroupIndeterminate(group.value)"
                    @change="toggleGroup(group)"
                    :aria-label="$t('role.toggleGroup', { group: group.name })"
                  />
                  <label :for="`group-${group.value}-table`">
                    {{ group.name }}
                  </label>
                </td>
                ...
                <td>
                  <input
                    ...
                    :aria-label="$t('role.togglePermission', { permission: permission.name })"
                  />
                </td>
              </tr>
            </template>
      ...
      <div v-else class="card-view">
        <div class="permission-groups">
          <div v-for="group in groupedPermissions" :key="group.value" class="permission-group">
